...
@app.route('/cameraresult', methods=['POST'])
def cameraresult():
    ...
    return render_template('cameraresult.html',enc_data=enc_data) 
        ...
        if file and allwed_file(file.filename):
            ...
            try:
                ...
                if prescription_shape_type == "A":#一段組の時
                    ...
                    print("基本情報読み取り結果")
                    print(str(text_top))
                    #text_place_path='./ocr_results/'+str(filename[:-4])+"_basic_ocr.txt" ~~txt file save comment out~~
                    ...
                    #text_place_path='./ocr_results/'+str(filename[:-4])+"_med_ocr.txt" ~~txt file save comment out~~
                    #f = open(text_place_path, 'w')
                ...
                #text_place_path='./ocr_results/'+str(filename[:-4])+"_basic_result.txt" ~~txt file save comment out~~
                #f = open(text_place_path, 'w')
                ...
                #text_place_path='./ocr_results/'+str(filename[:-4])+"_med_result.txt" ~~txt file save comment out~~
                #f = open(text_place_path, 'w')
...
def recept():
    if request.method == 'POST':
        ...
        birthday_gengou = dict_gengou_ids[result[4]]

        birthday=result[5]
        _13_patient_birthday= str(v1_b.convert_days(birthday,birthday_gengou))

        dict_patient_sex_ids = {'男':'1','女':'2'}
        _12_patient_sex = dict_patient_sex_ids[result[6]]

        dict_insurance_type_ids = {'被保険者':'1','被扶養者':'2'}
        _23_insurance_type = dict_insurance_type_ids[result[7]]
            

        prescription_gengou = dict_gengou_ids[result[8]]
